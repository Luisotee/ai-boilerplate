...
- **User Whitelist**: `WHITELIST_PHONES` env var — comma-separated. Empty = all users allowed (disabled). Each entry matches **either as a phone number or as a verbatim chat id**, parsed into two sets by `utils/whitelist.ts` (TS ×3) / `_parse_whitelist` (`routes/chat.py`) and matched by `isWhitelisted` / `_is_whitelisted`. Phone entries tolerate `+`, spaces, `-`, `()`, `.` and a trailing `@s.whatsapp.net`, and are stored as bare digits; anything containing `@` or starting with `tg:` is kept verbatim only, so `tg:-100…` can never be mistaken for a phone. Every entry is *also* kept verbatim, which is what keeps a bare `120363…` matching a group and a bare LID matching its chat on existing installs. **A group is matchable only by its `…@g.us` JID** — the matcher ignores the phone argument for group JIDs, so a member's whitelisted phone never admits the whole group. When set, non-whitelisted messages are silently ignored. Checked at both WhatsApp client level (primary) and AI API level (defense in depth). **Editing the whitelist via `PATCH /admin/settings` updates only the AI-API layer** — the TS clients keep their startup env value until restarted. So a `/admin` change takes full effect for *removals* near-immediately (≤ ~10s in the stream worker, which reads through the runtime_config TTL cache; instant in the API process) — the AI-API layer rejects them regardless of the client — but a *newly added* JID is still blocked by the client's primary filter until its env is updated + restart, unless that client's `WHITELIST_PHONES` is empty/disabled
