...
6. `routes/chat.py` intercepts slash commands (`/settings`, `/tts`, `/stt`, `/clean`, `/memories`, `/broadcast`, `/help`) before queuing
...
**Shared-group tools (`SHARED_GROUP_TOOLS_ENABLED`, default `false`, hot via `/admin`)**: two agent tools in `agent/tools/group_context.py`, both hidden from the model by a pydantic-ai `prepare` hook when the flag is off (their prompt section, `shared_group_guidance` in `agent/core.py`, follows the same flag):
- `get_group_context` — in a PRIVATE chat, list the groups the user shares with the bot, read one's transcript (`limit` / `since_hours`, via `get_conversation_messages` keyed by the group's row, looked up without creating one; rendered by `format_transcript` with `BOT_NAME`), or search it (`search_conversation_fn` scoped to the group row). Matching is lenient (exact → substring → fuzzy)
- `send_group_message` — relay a message into a shared group. Strict matching (`strict=True`: exact or one unambiguous substring, never fuzzy, duplicate names refused), always prefixed `📩 <name> (<phone>) asked me to send this message:` (name from `sender_name` / `User.name`, phone from the DB row; refuses if neither), sent through the existing `send-text` route (group JIDs pass through `normalizeJid`). The prompt makes the agent confirm group + exact text first
- **Privacy model**: the requester's identity always comes from their own `User` row (`deps.user_id`), never from tool arguments; a model-supplied group name is only matched against the freshly resolved shared list (`services/shared_groups.py` `resolve_shared_groups`, dispatched on `AgentDeps.client_id`). Baileys resolves membership server-side on every call (`POST /whatsapp/shared-groups` → `services/groups.ts` `findSharedGroups`, the 60s fleet snapshot in `group-cache.ts`; a failed lookup is a 500, never an empty list). Telegram cannot enumerate members, so its list is DERIVED from `conversation_messages.sender_jid` (groups the user has posted in; capped at 50) — incomplete for silent members, and never expiring, so `send_group_message` calls `POST /whatsapp/group-member` (`getChatMember`; `restricted` counts only with `is_member`) right before sending and **fails closed** on any error. Cloud API webhooks carry no group context: both tools answer `CLOUD_UNSUPPORTED`. Inside a group chat both refuse
- **Why off by default**: it copies group transcripts into private-chat model context (a second trip to the LLM provider, in a context the other members never see) and lets one member post into a group via the bot. An operator should opt in knowingly
- The Telegram client now sends the group title as `profile_name`, so Telegram group rows carry a name the tools can match

### Broadcasts (`/admin/broadcasts`, FleetView-driven)

An operator sends one message (changelog/announcement) to every chat the bot has ever talked to — no recency filter. `routes/broadcasts.py` snapshots recipients into `broadcast_recipients` at creation; the stream worker's third task, `streams/broadcast_consumer.py`, delivers them. Decisions (routing, audience, pacing math) are pure functions in `services/broadcast.py`.
- **Audience** (`private` default | `groups` | `all`) and **platforms** (subset of `baileys|cloud|telegram`; omitted = every client whose `GET /health` answers — URLs have localhost defaults, so "configured" can't be read from settings; an explicitly selected unreachable one is a 400). Opted-out chats and non-whitelisted chats (same rule as `_is_whitelisted`, any identity of a linked user counts) are excluded
- **Routing**: the platform comes from `users.last_client_id` (set on every `/chat/enqueue`; NULL on old rows → `tg:` = Telegram, else Baileys). A `/link`-merged user goes out on the platform they last used, or through their other identity if only that platform is selected. **Cloud API only reaches chats whose last inbound message is < 23h old** (free-form outside 24h needs a template, not implemented) — others are recorded `skipped`/`cloud_window`, re-checked at send time
- **Opt-out, opted in by default**: `users.broadcast_opt_out` (on `users`, not preferences, so `/clean all` never re-subscribes). Three ways: the footer (`broadcast_footer`, snapshotted per broadcast), `/broadcast on|off` (in `ADMIN_ONLY_COMMANDS`), and the `set_broadcast_subscription` agent tool ("stop sending me updates"). In a group only an admin may change it: the tool requires `AgentDeps.is_group_admin is True`, which is threaded `ChatRequest` → stream job field → `process_chat_job_direct` → `AgentDeps`. Both clients now resolve admin status for **every addressed group message** (Baileys: live `groupMetadata` for slash commands, the 5-min cache otherwise — `services/group-admin.ts`; Telegram: `getChatMember`), never for saved-only chatter. Telegram 403 (blocked/deactivated) also opts the chat out
- **One sender fleet-wide**: a Redis lease (`broadcast:lock`, 60s, heartbeat 20s, compare-and-extend Lua) — two senders would double the rate on one WhatsApp account. One broadcast queued/running at a time (409 otherwise). State is in Postgres, not a stream: pause/resume/cancel just change `broadcasts.status`, and each lane re-reads it before every send and while sleeping (15s slices)
- **Baileys anti-ban pacing** (hot `broadcast_*` settings, read per message): jittered 20-60s gap, "typing…" (`POST /whatsapp/typing`, 2-8s by length) before each message, a 600s ±30% pause every 15, a rolling 24h cap of 150 **across broadcasts** (counted from `sent_at`), a 09:00-21:00 window in `broadcast_timezone`, sends to the stored full JID (no `onWhatsApp` lookup burst). 5 consecutive non-404 failures pause it (`consecutive_failures`) — a failure spike is often the start of a restriction; 30 min of 503s pause it (`client_disconnected`). Telegram (50ms) and Cloud (200ms) lanes run concurrently and fast. 1000 users at the defaults ≈ a week; `POST /preview` returns `estimated_baileys_seconds`
- **Outcomes** (`error_code`): `not_on_whatsapp` (404), `blocked` (Telegram 403; the Telegram client's `sendErrorResponse` now maps `GrammyError` 403 → 403), `invalid_address` (400), `http_<n>`/`transport_error` (retried to 3 attempts), `opted_out`/`user_deleted`/`cloud_window` (skipped). A sent message is saved as an `assistant` row in that chat's history so a reply like "what's this?" has context. A crash between send and record re-sends that one message on restart (accepted)
- **FleetView contract**: `BroadcastStatus` (`queued|running|paused|completed|cancelled`), `BroadcastRecipientStatus` (`pending|sent|failed|skipped`), `BroadcastAudience`, `BroadcastPlatform` in `schemas.py` are closed enums once FleetView mirrors them. `idempotency_key` makes a timed-out create safe to retry (replay answers 200 with the original). `GET /admin/broadcasts/{id}` is rate-limit exempt for polling

### Telegram Message Flow (telegram-client)

1. Telegram → webhook `POST /webhook` → grammY `webhookCallback(bot, "fastify", { secretToken })` verifies the `X-Telegram-Bot-Api-Secret-Token` header. **Or**, with `TELEGRAM_MODE=polling`, long polling (`src/polling.ts`): `deleteWebhook()` then `run(bot)` from `@grammyjs/runner` (not awaited); no `/webhook` route is registered and no public URL is needed
...
4. Group messages: `utils/mention.ts` checks for `@bot` mention (case-insensitive), `text_mention` entity for `bot.botInfo.id`, a leading `bot_command` (`/cmd` or `/cmd@thisbot`), or reply to a bot message — otherwise save-only. Every addressed group text message also resolves the sender's admin status via `getChatMember` (`services/group-admin.ts`)
...
pnpm dev:queue                          # Start the stream worker (chat + PDF + broadcast consumers; required for replies, PDF processing AND broadcasts)
...
- **Group gating (`GROUP_GATING=jid|membership`, default `jid`)**: selects how a *group* gets in scope when `WHITELIST_PHONES` is set. Read by the AI API (`config.py`, display-only in `runtime_config`) and by the Baileys + Telegram clients (`config.groupGating`) — keep them equal via the root `.env`. The truth table is `utils/gating.ts` (`gateMessage` / `decideGroupGating`), byte-identical in whatsapp-client and telegram-client (enforced by `whitelist-copies.test.ts`); in `jid` mode the callers pass `groupAllowed = groupExplicit`, which reproduces the historical gate exactly
  - `jid`: a group is admitted only by its own id; then every member may address the bot
  - `membership` (Baileys): a group is in scope if it is listed, the sender is whitelisted, or ANY participant is whitelisted (`groupHasWhitelistedMember` in `services/groups.ts`: participant `id`/`lid`/`phoneNumber` through the whitelist matcher, then a quiet `getPNForLID` fallback for LID-only participants; metadata from `group-cache.ts`, bounded at 10s, **fails closed**). The bot saves every message of an in-scope group but replies only to a whitelisted sender (`gate.suppressResponse` → `saveOnly`), unless the group is listed explicitly. The gate still runs between `resolveSenderPhone` and `sock.user!.id` (`resolveGate` in `whatsapp.ts`; pinned by `whatsapp-whitelist-placement.test.ts` for both modes)
  - `membership` (Telegram): the Bot API cannot enumerate members, so every group the bot is in is in scope and saved; replies are gated per sender (`tg:<from.id>`) unless the group id is listed (`resolveGate` in `updates.ts`)
  - AI API: in `membership` mode `_is_whitelisted` admits every group JID (`is_group_jid`: `@g.us` or `tg:-…`) — the client is authoritative for groups because only it can see membership; 1:1 chats are enforced in both modes
...
- **10 tables**: users, conversation_messages, conversation_preferences, core_memories, knowledge_base_documents, knowledge_base_chunks, bot_prompt, runtime_settings, broadcasts, broadcast_recipients
- **No Alembic migrations** — uses SQLAlchemy `create_all()`, which only creates missing *tables*, never new columns. Column/index changes ship as hand-written, idempotent (`IF NOT EXISTS`) SQL in `packages/ai-api/docs/migrations/<YYYY-MM-DD>-<name>.sql`, named so they match what `create_all()` emits on a fresh DB. Apply them to an existing Docker deployment BEFORE rolling out the code that needs them (safe to re-run):
  ...
  ```
  (`aiagent-postgres` is the compose `container_name`, `${SERVICE_NAME}-postgres`; substitute `<SERVICE_NAME>-postgres` and your `POSTGRES_USER` / `POSTGRES_DB` if you changed the defaults.) Current migrations: `2026-09-21-kb-file-hash.sql`, `2026-09-25-broadcast.sql` (`users.broadcast_opt_out`, `users.last_client_id`; the `broadcasts`/`broadcast_recipients` tables come from `create_all()`)
...
- Models: `database.py` (users, messages, preferences, core_memories, bot_prompt, runtime_settings, broadcasts, broadcast_recipients) + `kb_models.py` (documents, chunks)
...
- Group admin check is lazy: only for addressed group messages (`resolveSenderGroupAdmin` in `services/group-admin.ts`) — a live `groupMetadata` fetch for slash commands, the 5-min `group-cache.ts` entry for other addressed messages (the broadcast opt-out tool needs it), never for saved-only chatter. `isSenderGroupAdmin` (`utils/message.ts`) matches `key.participant`/`participantAlt` against each member's `id`/`lid`/`phoneNumber` — comparing only `id` to `participant` missed admins whenever LID vs phone addressing differed, and the API now fails closed on anything but `true`
...
### Telegram (TS)
- **`TELEGRAM_MODE=webhook|polling` (default `webhook`; webhook mode is byte-for-byte the old behaviour)**. Polling-only machinery, all gated on the mode in `bot.ts` / `main.ts` / `polling.ts`:
  - **Runner, not `bot.start()`**: grammY's built-in polling handles updates strictly sequentially, and one reply can block on the AI API for up to `POLL_MAX_DURATION_MS`, stalling every other chat. The runner is concurrent (`TELEGRAM_POLL_CONCURRENCY`); `sequentialize(ctx => ctx.chat?.id.toString())` restores per-chat order and is registered before `autoChatAction`
  - **`deleteWebhook()` before polling**: a registered webhook makes every `getUpdates` fail with 409 Conflict, permanently. Two processes polling one token also 409 — use a separate bot token per environment
  - **`autoRetry` must keep `rethrowHttpErrors: true` in polling mode.** It sits UNDER the runner's `getUpdates`, and its `HttpError` branch retries in an inner loop that never decrements `maxRetryAttempts`: a network partition would be swallowed forever, the runner's `maxRetryTime` (`TELEGRAM_POLL_MAX_RETRY_MS`, 5 min; its own default is 15 h) would never apply, `runner.task()` would never reject, and the bot would go deaf while healthy. Webhook mode keeps the old options
  - **Health is not a one-way latch**: `runner.task()` rejecting → `markBotDisconnected()` (health/readiness flip) → `shutdownWithError` → exit 1 → Docker restarts
  - **`runner.stop()` does NOT drain in-flight handlers**, despite its JSDoc: it resolves once the sink has a free slot. Shutdown is `stopLongPolling`: `stop()`, then poll `runner.size()` until 0 (bounded by `TELEGRAM_SHUTDOWN_DRAIN_MS`, default `POLL_MAX_DURATION_MS + 10s`), and only then `app.close()` — the AI API calls back into the HTTP server while handlers run. Compose sets `stop_grace_period: 140s` so Docker doesn't SIGKILL the drain
  - **Error boundary**: an outermost middleware that swallows + reports (`reportBotError`, Sentry) and a `bot.catch` backstop — in polling mode an escaping error reaches the runner's sink (and `unhandledRejection` → exit). Webhook mode deliberately has neither: errors propagate to `routes/webhook.ts`, which logs them and answers 500 so Telegram retries
  - `bot.init()` still runs before polling: `isAddressedToBot` compares against `botInfo.username`, so a bot that polls before init is silently deaf in groups
  - `validateRequiredEnv` rejects an unknown `TELEGRAM_MODE`; the webhook-secret requirement applies only in webhook mode
...
- **Privacy mode must be OFF** (via `@BotFather` → `/setprivacy` → Disable) for the bot to see non-addressed group messages. After toggling, **the bot must be removed and re-added** to existing groups — Telegram caches privacy state on join. With privacy mode ON the bot never sees un-addressed group messages at all, so group transcripts (GROUP_GATING=membership saving, the shared-group tools, Telegram shared-group discovery) are empty
...
- Slash commands (`/settings`, `/tts`, `/stt`, `/clean`, `/memories`, `/broadcast`, `/help`) are intercepted in `routes/chat.py` — they never reach the AI agent
