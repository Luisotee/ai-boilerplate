...
class JobStatusResponse(BaseModel):
    ...
    chunks: List[ChunkData] = Field(default_factory=list, description="Accumulated streaming chunks")
    total_chunks: int = Field(default=0, description="Total number of chunks available")
    complete: bool = Field(default=False, description="Whether job has finished (success or failure)")
